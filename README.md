...
除常规的余额天数与金额阈值外，预算预警还会按中国法定节假日安排做覆盖检查：若预计余额会进入假期、却无法覆盖到节后首个可充值工作日并额外保留 1 天缓冲，会立即发出“节假日风险”提醒。此类升级提醒不受普通 24 小时去重限制；节假日日期与调休工作日以本地年度配置维护。
...
| Jelenew+ | performance | 1614799860253372 |
| Jelenew-Brand & Lab | brand | 568835832834495 |

预算预警 `--check-budget` 只监控三个 `performance` 账户。Morning Report V1 会遍历全部四个账户。
